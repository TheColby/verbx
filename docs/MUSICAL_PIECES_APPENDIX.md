...
For each work, listen once without analysis. On the second pass, mark the direct sound, first audible reflection, apparent room size, decay color, stereo or multichannel motion, and moments when ambience becomes an instrument rather than a background. On a third pass, build a short verbx study using your own source material. The aim is not to counterfeit a copyrighted recording; it is to isolate and understand an acoustic or production principle.

When a named recording is central to the entry, use that recording or a documented reissue. When the composition itself is central, compare performances made in contrasting rooms. Level-match comparisons, protect your monitoring, and remember that streaming normalization can conceal large differences in crest factor and perceived depth.
...
1. **One source, six eras.** Render the same eight-bar vocal or percussion phrase using slapback, chamber, dub throw, gated room, ambient tail, and contemporary sparse vocal settings. Loudness-match all versions and record the parameters in JSON.
2. **Architecture versus algorithm.** Compare a measured church or hall IR with an algorithmic preset at the same broadband RT60. Note differences in early-reflection pattern, modal coloration, stereo stability, and high-band decay.
3. **Depth without width.** Keep the output mono while changing pre-delay, DRR, damping, and early density. Restore stereo only after the front-to-back hierarchy works.
4. **Silence as orchestration.** Choose a sparse work from this appendix and reproduce its spacing with your own material. Do not add ambience until the dry arrangement leaves enough time for a decay to be heard.
5. **Finite versus frozen.** Compare a 30-second finite decay, a 360-second extreme decay, and freeze. Measure peak growth and spectral accumulation over at least one minute.
6. **Version audit.** Compare two recordings or mixes of the same composition. Document which differences arise from performance venue, microphone distance, mix decisions, mastering, or playback format.
