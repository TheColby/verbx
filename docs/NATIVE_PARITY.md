...
| Presets | Built-in/generated Python preset bank | 260 host programs with persisted editable labels and editor browser | Partial | Four immediate references plus 256 deterministic room, hall, plate, spring, chamber, drone, shimmer, and tight variations recall the realtime parameter set; the plug-in browser stays synchronized with host programs. Python preset-bank parity remains deferred. |
