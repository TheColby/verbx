...
Use `--execution-profile` to auto-tune worker count by workload pattern (`auto`,
`cpu-balanced`, `io-bound`, `max-throughput`) when `--jobs 0` is used.
Use `--retries` to auto-retry failed variants, then inspect summary throughput fields
(`elapsed_seconds`, `outputs_per_second`, `total_attempts`) plus stage telemetry
(`read_seconds`, `process_seconds`, `write_seconds`) for capacity planning.
