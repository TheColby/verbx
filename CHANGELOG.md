...
- SOFA CLI utilities: `ir sofa-inspect` and `ir sofa-convert`.
...
- `batch corpus-generate` adds execution profiles and stage telemetry (`read/process/write` timing).
- CI now runs dedicated docs-consistency checks (`tests/test_docs_sync.py`).
- Ambisonics render validation now supports HOA up to order 7.
