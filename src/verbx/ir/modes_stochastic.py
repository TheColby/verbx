...
def generate_stochastic_ir(
    ...
    noise = rng.standard_normal((n, ch), dtype=np.float64)
    ...
    envelope = np.power(10.0, (-3.0 * t) / rt_curve).astype(np.float64)

    out = noise * envelope[:, np.newaxis] * np.float64(np.clip(density, 0.01, 3.0))
    ...
    alpha = np.float64(0.02 + (0.95 * damp))
    state = np.zeros(ch, dtype=np.float64)
    for i in range(n):
        state = ((1.0 - alpha) * out[i, :]) + (alpha * state)
        out[i, :] = state

    out[0, :] += np.float64(1.0)
