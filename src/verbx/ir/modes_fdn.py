...
def generate_fdn_ir(
    ...
    engine = AlgoReverbEngine(
        AlgoReverbConfig(
            ...
            fdn_spatial_coupling_mode=fdn_spatial_coupling_mode.strip()
            .lower()
            .replace("-", "_"),
            fdn_spatial_coupling_strength=float(np.clip(fdn_spatial_coupling_strength, 0.0, 1.0)),
            fdn_nonlinearity=fdn_nonlinearity.strip().lower().replace("-", "_"),
            fdn_nonlinearity_amount=float(np.clip(fdn_nonlinearity_amount, 0.0, 1.0)),
            fdn_nonlinearity_drive=float(np.clip(fdn_nonlinearity_drive, 0.1, 8.0)),
            room_size_macro=float(np.clip(room_size_macro, -1.0, 1.0)),
            clarity_macro=float(np.clip(clarity_macro, -1.0, 1.0)),
            warmth_macro=float(np.clip(warmth_macro, -1.0, 1.0)),
            envelopment_macro=float(np.clip(envelopment_macro, -1.0, 1.0)),
            wet=1.0,
            dry=0.0,
            block_size=4096,
        )
