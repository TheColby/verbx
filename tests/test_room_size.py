...
from verbx.analysis.room_size import classify_room, estimate_room_size, infer_absorption
...
def _make_reverberant(sr: int, rt60: float, duration: float = 3.0) -> np.ndarray:
    ...
    clean = np.asarray(sosfilt(sos, noise), dtype=np.float64)
...
def _metric_float(result: dict[str, object], key: str) -> float:
    value = result[key]
    assert isinstance(value, (int, float)), f"{key} expected numeric value, got {type(value)}"
    return float(value)
...
def test_all_numeric_values_are_finite() -> None:
    ...
    for k in float_keys:
        v = result[k]
        value = _metric_float(result, k)
        assert np.isfinite(value), f"{k} = {v} is not finite"
...
def test_volume_grows_with_rt60(rt60: float) -> None:
    ...
    result = estimate_room_size(audio, sr)
    assert _metric_float(result, "room_volume_m3") > 0.0
...
def test_larger_rt60_gives_larger_volume() -> None:
    ...
    v_short = _metric_float(short, "room_volume_m3")
    v_long = _metric_float(long_, "room_volume_m3")
...
def test_dimensions_consistent_with_volume() -> None:
    ...
    result = estimate_room_size(audio, sr)
    w = _metric_float(result, "room_dim_width_m")
    d = _metric_float(result, "room_dim_depth_m")
    h = _metric_float(result, "room_dim_height_m")
    v = _metric_float(result, "room_volume_m3")
...
def test_confidence_interval_bounds_volume() -> None:
    ...
    result = estimate_room_size(audio, sr)
    v = _metric_float(result, "room_volume_m3")
    v_low = _metric_float(result, "room_volume_m3_low")
    v_high = _metric_float(result, "room_volume_m3_high")
...
def test_critical_distance_is_positive() -> None:
    ...
    audio = _make_ir(sr, 1.0, duration=4.0)
    result = estimate_room_size(audio, sr)
    assert _metric_float(result, "room_critical_distance_m") > 0.0
...
def test_classify_room_labels(volume_m3: float, rt60: float, expected: str) -> None:
    assert classify_room(volume_m3, rt60) == expected
...
def test_infer_absorption_hard_surface() -> None:
    ...
    alpha = infer_absorption(rt60_low=2.0, rt60_mid=1.0, rt60_high=0.5)
    assert alpha < 0.25


def test_infer_absorption_treated_surface() -> None:
    # High HF/LF ratio → treated → higher alpha
    alpha = infer_absorption(rt60_low=1.5, rt60_mid=1.4, rt60_high=1.3)
    assert alpha > 0.30


def test_infer_absorption_fallback_for_zeros() -> None:
    alpha = infer_absorption(rt60_low=0.0, rt60_mid=0.0, rt60_high=0.0)
...
def test_prior_absorption_overrides_inference() -> None:
    ...
    assert _metric_float(r_override, "room_volume_m3") != _metric_float(r_default, "room_volume_m3")
